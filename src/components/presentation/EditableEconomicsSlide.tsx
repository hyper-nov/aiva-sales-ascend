        ...
        <div className="grid grid-cols-2 gap-12 max-w-6xl mx-auto items-start">
          ...
          <div className="space-y-6">
            <div className="text-center">
              <EditableText
                as="h2"
                className="text-2xl font-light text-slate-700 mb-4"
                isEditing={isEditMode}
                onSave={(text) => updateText('beforeTitle', text)}
              >
                {currentTexts.beforeTitle || 'До AIVA'}
              </EditableText>
            ...
            <div className="bg-gradient-to-br from-slate-50 to-slate-100/50 backdrop-blur-sm rounded-3xl p-8 border border-slate-200/50 space-y-6">
              {beforeCosts.map((cost, index) => (
                <div key={index} className="flex justify-between items-start group hover:bg-white/50 rounded-xl p-3 transition-all duration-300">
                  <EditableText
                    as="span"
                    className="text-sm text-slate-700 flex-1 pr-4 leading-relaxed"
                    isEditing={isEditMode}
                  ...
                  >
                    {currentTexts[`beforeAmount${index}`] || `${formatNumber(parseInt(cost.amount.replace(/\s/g, '')))} ₽`}
                  </EditableText>
                </div>
              ))}
              ...
              <div className="border-t border-slate-300 pt-6">
                <div className="flex justify-between items-center bg-white/70 rounded-2xl p-4">
                  <EditableText
                    ...
                    isEditing={isEditMode}
                    onSave={(text) => updateText('beforeTotal', text)}
                  >
                    {currentTexts.beforeTotal || 'ИТОГО:'}
                  </EditableText>
          ...
          <div className="space-y-6">
            <div className="text-center">
              <EditableText
                ...
                className="text-2xl font-light text-slate-700 mb-4"
                isEditing={isEditMode}
                onSave={(text) => updateText('afterTitle', text)}
              >
                {currentTexts.afterTitle || 'С AIVA'}
              </EditableText>
            ...
            <div className="bg-gradient-to-br from-blue-50 to-cyan-50/50 backdrop-blur-sm rounded-3xl p-8 border border-blue-200/50 space-y-6">
              {afterCosts.map((cost, index) => (
                <div key={index} className="flex justify-between items-start group hover:bg-white/50 rounded-xl p-3 transition-all duration-300">
                  ...
                  >
                    {currentTexts[`afterAmount${index}`] || (cost.amount === "0" ? "0 ₽" : `${formatNumber(parseInt(cost.amount.replace(/\s/g, '')))} ₽`)}
                  </EditableText>
                </div>
              ))}
              ...
              <div className="border-t border-blue-200 pt-6">
                <div className="flex justify-between items-center bg-gradient-to-r from-blue-100/70 to-cyan-100/70 rounded-2xl p-4">
                  <EditableText
                    ...
                    className="text-lg font-medium text-slate-900"
                    isEditing={isEditMode}
                    onSave={(text) => updateText('afterTotal', text)}
                  >
                    {currentTexts.afterTotal || 'ИТОГО:'}
                  </EditableText>
          ...
          <div className="bg-gradient-to-r from-slate-900 via-blue-900 to-cyan-900 rounded-3xl p-8 border border-slate-700 relative overflow-hidden">
            ...
            <div className="relative z-10 text-center space-y-4">
              ...
              <EditableText
                ...
                isEditing={isEditMode}
                onSave={(text) => updateText('savingsText', text)}
              >
                {currentTexts.savingsText || 'AIVA не просто дешевле, она сразу же приносит дополнительную прибыль за счет роста эффективности'}
              </EditableText>
            </div>
          </div>
        </div>
